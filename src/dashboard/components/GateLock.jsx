...
function GateLock({ user, application, onRefresh }) {
      ...
      const checkStatus = async () => {
        try {
          ...
          if (res.data?.status === 'completed') {
            setPolling(false);
            toast.success('Registration payment completed & verified successfully!');
  ...
  const handleSendRequest = async (e) => {
    e.preventDefault();
    if (dept === 'none') {
      toast.error('Please select a valid department track.');
      return;
    }
    setLoading(true);
    try {
      ...
      toast.error(err.response?.data?.message || 'Failed to submit internship application.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleInitiatePayment = async (e) => {
    ...
    try {
      ...
      setPolling(true);
      toast.info('Payment initiated! Please check your phone for the Mobile Money prompt to approve.');
    } catch (err) {
      toast.error(err.response?.data?.message || 'Failed to initiate registration payment.');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="w-full max-w-lg rounded-3xl border border-white/10 bg-slate-950/80 p-8 shadow-[0_24px_80px_rgba(0,0,0,0.6)] backdrop-blur-2xl transition duration-500 animate-fade-up">
        ...
        <div className="text-center">
          ...
          <p className="mt-2 text-sm leading-6 text-slate-400">
            Please complete the onboarding steps below to unlock your active candidate cockpit.
          </p>
        </div>
        ...
        <div className="mt-8 grid grid-cols-2 gap-4">
          <div className={`rounded-2xl border p-4 text-left transition ${application ? 'border-emerald-500/30 bg-emerald-500/5' : 'border-blue-500/25 bg-blue-500/5'}`}>
            <span className="text-[0.6rem] font-bold uppercase tracking-wider text-slate-500 block">Step 1</span>
            <strong className="mt-1 block text-sm font-black text-white">Internship Request</strong>
          ...
          <div className={`rounded-2xl border p-4 text-left transition ${application?.paymentStatus === 'paid' ? 'border-emerald-500/30 bg-emerald-500/5' : step === 'payment' ? 'border-amber-500/30 bg-amber-500/5' : 'border-white/5 bg-white/[0.02]'}`}>
            <span className="text-[0.6rem] font-bold uppercase tracking-wider text-slate-500 block">Step 2</span>
            <strong className="mt-1 block text-sm font-black text-white">Registration Fee</strong>
            <span className={`mt-2 inline-flex items-center gap-1.5 text-[0.68rem] font-black uppercase tracking-wider ${application?.paymentStatus === 'paid' ? 'text-emerald-400' : polling ? 'text-amber-400 animate-pulse' : 'text-slate-500'}`}>
              ...
              {application?.paymentStatus === 'paid' ? 'Paid' : polling ? 'Processing' : 'Pending'}
            </span>
          </div>
        </div>
        ...
        <div className="mt-8 border-t border-white/10 pt-6">
          ...
          {step === 'request' && (
            <form onSubmit={handleSendRequest} className="space-y-4">
              <div>
                <h3 className="text-sm font-black text-white uppercase tracking-wider mb-2">1. Submit Internship Request</h3>
                <p className="text-xs text-slate-500">Choose your dynamic technology department track & remote mode preferences.</p>
              </div>

              <label className="block">
                <span className="mb-2 block text-xs font-bold text-slate-400">Department Track</span>
              ...
              <div>
                <span className="mb-2 block text-xs font-bold text-slate-400">Application Track</span>
                <div className="grid grid-cols-2 gap-3">
                  {[
                    ['online', 'Remote Track', 'Online evaluation.'],
                    ['on_site', 'In-Person Track', 'Physical placement.']
              ...
              <button type="submit" disabled={loading} className="btn-primary h-[50px] w-full justify-center rounded-2xl transition disabled:opacity-60 disabled:cursor-not-allowed">
                {loading ? 'Submitting...' : 'Send Internship Request →'}
              </button>
            </form>
          )}
          ...
          {step === 'payment' && (
            <form onSubmit={handleInitiatePayment} className="space-y-4">
              <div>
                <h3 className="text-sm font-black text-white uppercase tracking-wider mb-2">2. Pay Registration Fee</h3>
                <p className="text-xs text-slate-500">Securely pay the 5,000 XAF registration fee online via Mobile Money to activate your candidate cockpit.</p>
              </div>

              <div className="rounded-2xl border border-emerald-400/20 bg-emerald-500/10 p-4">
                <div className="flex items-center justify-between gap-4">
                  <span className="text-xs font-black text-emerald-300">Registration Payment Required</span>
                ...
                <p className="mt-1.5 text-[0.68rem] font-semibold leading-relaxed text-emerald-100/70">
                  Online payments are processed automatically.
                </p>
              </div>
              ...
              {!polling ? (
                <>
                  <label className="block">
                    <span className="mb-2 block text-xs font-bold text-slate-400">Mobile Money Phone Number</span>
                    <input type="tel" required placeholder="+237 6XX XXX XXX" value={phone} onChange={(e) => setPhone(e.target.value)} className={inputCls} />
                  </label>

                  <button type="submit" disabled={loading} className="btn-primary h-[50px] w-full justify-center rounded-2xl transition disabled:opacity-60 disabled:cursor-not-allowed">
                    {loading ? 'Initiating Payment...' : 'Pay Registration Fee Now'}
                  </button>
                </>
              ) : (
                <div className="rounded-2xl border border-blue-400/25 bg-blue-500/10 p-6 text-center space-y-4">
                  ...
                  <strong className="block text-sm font-bold text-white">Waiting for payment confirmation...</strong>
                  <p className="text-xs leading-relaxed text-slate-400">
                    A Mobile Money confirmation prompt was sent to your phone (<strong className="text-white">{phone}</strong>). Please enter your PIN to approve the transaction. We are monitoring status.
                  </p>
          ...
          {step === 'pending_approval' && (
            <div className="space-y-5 text-center py-2 animate-fade-up">
              <div>
                <h3 className="text-sm font-black text-white uppercase tracking-wider mb-2">3. Admin Review & Allocation</h3>
                <p className="text-xs text-slate-500">Your application is fully registered and online payment has been verified!</p>
              </div>
              ...
              <div className="rounded-2xl border border-emerald-400/20 bg-emerald-500/10 p-5 text-left flex items-start gap-4">
                ...
                <div className="min-w-0">
                  <strong className="block text-sm text-emerald-300 font-black">Registration Complete</strong>
                  <p className="mt-1 text-xs leading-relaxed text-emerald-100/70">
                    We received your payment of 5,000 XAF for your registration.
                  </p>
                </div>
              </div>
              ...
              <div className="rounded-2xl border border-amber-500/20 bg-amber-500/5 p-5 text-left flex items-start gap-4">
                ...
                <div className="min-w-0">
                  <strong className="block text-sm text-amber-300 font-black">Pending Operational Review</strong>
                  <p className="mt-1 text-xs leading-relaxed text-slate-400">
                    Your internship request for <strong className="text-white">{application?.department || dept}</strong> is being processed by our administrative staff. They will allocate your dynamic training class and assign your direct supervisor shortly.
                  </p>
                </div>
              </div>
